...
public class ServiceAutoActivity extends AppCompatActivity {
    ...
    private void init() {
        ...
        mServiceAuto = new ServiceAuto(
                ...
                intent.getIntExtra("seenOffer", 3)
        );
    ...
    private void addAppointment() {
        ...
        if (!isLoggedIn) {
            ...
            scheduleDescriptionTitle.setText("Descrie motivul pentru care faci programare");
            ...
            final View scheduleDescriptionContent = getLayoutInflater().inflate(R.layout.appointment_description_popup_layout, null);
            ...
            if ((mServiceAuto.getType() & 1) != 1) {
                scheduleRepairServiceCheck.setVisibility(View.GONE);
            }
            if ((mServiceAuto.getType() & 2) != 2) {
                scheduleServiceTireCheck.setVisibility(View.GONE);
            }
            if ((mServiceAuto.getType() & 4) != 4) {
                scheduleServiceChassisCheck.setVisibility(View.GONE);
            }
            if ((mServiceAuto.getType() & 8) != 8) {
                scheduleServiceItpCheck.setVisibility(View.GONE);
            }
            scheduleRepairServiceCheck.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
                @Override
                public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                    if (isChecked) {
                        scheduleServiceTireCheck.setChecked(false);
                        scheduleServiceChassisCheck.setChecked(false);
                        scheduleServiceItpCheck.setChecked(false);
                    }
                }
            });
            scheduleServiceTireCheck.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
                @Override
                public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                    if (isChecked) {
                        scheduleRepairServiceCheck.setChecked(false);
                        scheduleServiceChassisCheck.setChecked(false);
                        scheduleServiceItpCheck.setChecked(false);
                    }
                }
            });
            scheduleServiceChassisCheck.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
                @Override
                public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                    if (isChecked) {
                        scheduleRepairServiceCheck.setChecked(false);
            ...
            scheduleServiceItpCheck.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
                @Override
                public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                    if (isChecked) {
                        scheduleRepairServiceCheck.setChecked(false);
                    ...
                    .setPositiveButton("Continua", new DialogInterface.OnClickListener() {
                        ...
                        public void onClick(DialogInterface dialog, int which) {
                            if (scheduleDescriptionText.getText().toString().trim().isEmpty() ||
                                ...
                                Toast.makeText(mCtx, "Trebuie completat câmpul din pop-up și o casuță bifată!", Toast.LENGTH_SHORT).show();
                            } else {
    ...
    private boolean verifyInputOnServerSide() {
        ...
        jsonBody.addProperty("serviceCity", mEditServiceCity.getText().toString().trim());
        ...
        byte[] data;
        BitmapDrawable bd = ((BitmapDrawable) mEditServiceImage.getDrawable());
        if (bd != null) {
            ...
            BitmapDrawable bd1 = (BitmapDrawable) ContextCompat.getDrawable(ServiceAutoActivity.this, R.drawable.default_service_image);
            Bitmap bitmap = bd1.getBitmap();
            ByteArrayOutputStream baos = new ByteArrayOutputStream();
            bitmap.compress(Bitmap.CompressFormat.JPEG, 100, baos);
            data = baos.toByteArray();
        }
                    ...
                    public void onSuccess(UploadTask.TaskSnapshot taskSnapshot) {
                        newImagePath.getDownloadUrl().addOnSuccessListener(new OnSuccessListener<Uri>() {
                            ...
                            public void onSuccess(Uri uri) {
                                ...
                                try {
                                    ...
                                    if (response.getHeaders().code() == 200) {
                                        ...
                                        mEditServiceProgressBar.setVisibility(View.GONE);
                                        mEditLinearLayout.setBackground(new ColorDrawable(Color.parseColor("#FFFFFF")));
                            ...
                            public void onFailure(@NonNull Exception e) {
                                Log.e(TAG, "onFailure: fail to get download link");
                                mEditServiceProgressBar.setVisibility(View.GONE);
                                mEditLinearLayout.setBackground(new ColorDrawable(Color.parseColor("#FFFFFF")));
                                setEnableFields(true);
                                Toast.makeText(mCtx, "Ceva nu a mers! Verifica conexiunea la internet!", Toast.LENGTH_SHORT).show();
    ...
    private void populateActivity() {
        ...
        if (mServiceAuto.getPriceService() != -1) {
            prices.append(String.format("Manoperă Service: %s Lei/h,", mServiceAuto.getPriceService()));
        }
        if (mServiceAuto.getPriceTire() != -1) {
            prices.append(String.format(" Vulcanizare: %s Lei schimbul de anvelope,", mServiceAuto.getPriceTire()));
        }
        if (mServiceAuto.getPriceChassis() != -1) {
            prices.append(String.format(" Manoperă Tinichigerie: %s Lei/h,", mServiceAuto.getPriceChassis()));
        }
